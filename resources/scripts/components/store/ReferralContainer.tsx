...
export default () => {
        ...
        deleteReferralCode(code)
            .then(() => {
                ...
                addFlash({
                    type: 'success',
                    key: 'referrals',
                    message: 'O código de referência foi excluído.',
                });
            })
            .catch((error) => clearAndAddHttpError(error))
            .then(() => {
    ...
    return (
        <PageContentBlock title={'Referências'}>
            <h1 className={'j-left text-5xl'}>Referências</h1>
            <h3 className={'j-left text-2xl mt-2 text-neutral-500'}>Crie um código e compartilhe-o com outras pessoas.</h3>
            <FlashMessageRender byKey={'referrals'} className={'mt-2'} />
            <Container className={'j-up lg:grid lg:grid-cols-3 my-10'}>
                <ContentBox title={'Seus códigos de referências'} css={tw`sm:mt-0`}>
                    <Dialog.Confirm
                        title={'Excluir código de referência'}
                        confirm={'Deletar código'}
                        open={!!code}
                        onClose={() => setCode('')}
                        onConfirmed={() => doDeletion(code)}
                    >
                        Os usuários não poderão mais usar essa chave para inscrição.
                    </Dialog.Confirm>
                    <SpinnerOverlay visible={loading} />
                    {codes.length === 0 ? (
                        <p css={tw`text-center my-2`}>{!loading && 'Não existem códigos de referência para esta conta.'}</p>
                    ) : (
                ...
                <ContentBox title={'Vantagens disponíveis'} css={tw`mt-8 sm:mt-0 sm:ml-8`}>
                    <h1 css={tw`text-xl`}>
                        Você receberá <span className={'text-green-500'}>{reward}</span> créditos para cada usuário que você
                        se referir a este Painel.
                    </h1>
                </ContentBox>
                <ContentBox title={'Usuários Encaminhados'} css={tw`mt-8 sm:mt-0 sm:ml-8`}>
                    <h1 css={tw`text-gray-400`}>Não é possível exibir estatísticas no momento.</h1>
                </ContentBox>
            </Container>
        </PageContentBlock>
    );
